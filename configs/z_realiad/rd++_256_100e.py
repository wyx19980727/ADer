...
class cfg(cfg_common, cfg_dataset_default, cfg_model_rd):
	...
	def __init__(self):
		...
		self.optim.distill_opt = Namespace()
		self.optim.lr = self.lr
		self.optim.proj_opt.kwargs = dict(name='adamw', betas=(0.9, 0.999), eps=1e-8, weight_decay=self.weight_decay, amsgrad=False)
		self.optim.distill_opt.kwargs = dict(name='adamw', betas=(0.9, 0.999), eps=1e-8, weight_decay=self.weight_decay, amsgrad=False)
