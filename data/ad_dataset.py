...
class RDPPAD(data.Dataset):
	def __init__(self, cfg, train=True, transform=None, target_transform=None):
		...
		if name in ['mvtec', 'coco', 'visa', 'medical', 'btad', 'mpdd', 'mad_sim', 'mad_real']:
			...
			for cls_name in self.cls_names:
				data_cls_all = []
				cls_info = json.load(open(f'/home/albus/DataSets/REAL-IAD/realiad_jsons/{cls_name}.json', 'r'))
				data_cls = cls_info['train' if self.train else 'test']
				for data in data_cls:
					if data['anomaly_class'] == 'OK':
						info_img = dict(
		...
		if self.use_sample == True:
			...
			for i in range(5):
				...
				if anomaly[i] == 0:
					...
					img_mask = np.array(self.loader_target(f'{self.root}/{mask_path[i]}')) > 0
					img_mask = Image.fromarray(img_mask.astype(np.uint8) * 255, mode='L')
				img = self.data_transforms(img)
				img_mask = self.target_transform(img_mask) if self.target_transform is not None and img_mask is not None else img_mask
				img_noise = self.data_transforms(img_noise)
				imgs.append(img.unsqueeze(0))
				img_noises.append(img_noise.unsqueeze(0))
				img_masks.append(img_mask.unsqueeze(0))
				# from torchvision.utils import save_image
				# save_image(img, 'example_image.png')
				# save_image(img_noise, 'example_image_noise.png')
				# import ipdb; ipdb.set_trace()
			sample_anomaly = 0 if 1 not in anomaly else 1
			img= torch.cat(imgs, dim=0)
			img_noise = torch.cat(img_noises, dim=0)
			img_mask = torch.cat(img_masks, dim=0)
			return {'img': img, 'img_noise': img_noise, 'img_mask': img_mask, 'cls_name': cls_name, 'anomaly': np.array(anomaly), 'img_path': img_path, 'sample_anomaly': sample_anomaly}
		else:
			data = self.data_all[index]
...
class RealIAD(data.Dataset):
	def __init__(self, cfg, train=True, transform=None, target_transform=None):
		...
		for cls_name in self.cls_names:
			data_cls_all = []
			#cls_info = json.load(open(f'/home/albus/DataSets/REAL-IAD/realiad_jsons_subsample0.1/{cls_name}.json', 'r'))
			cls_info = json.load(open(f'/home/albus/DataSets/REAL-IAD/realiad_jsons/{cls_name}.json', 'r'))
			data_cls = cls_info['train' if self.train else 'test']
			for data in data_cls:
				if data['anomaly_class'] == 'OK':
	...
	def __getitem__(self, index):
		if self.use_sample == True:
			...
			for i in range(5):
				...
				# aug_img.append(self.loader(img_path[i]))
				if anomaly[i] == 0:
				...
				#import ipdb; ipdb.set_trace()
				''''''
				# aug_img[i] = self.aug_transform(img[i]) if self.aug_transform is not None and self.train else img[i]
				# aug_img[i] = aug_img[i].unsqueeze(0) if self.aug_transform is not None and self.train else None
				''''''
			...
			''''''
			# aug_img= torch.cat(aug_img, dim=0) if self.aug_transform is not None and self.train else None
			label = torch.Tensor([int(self.cls2idx[name]) for name in cls_name])
			''''''
			#import ipdb; ipdb.set_trace()
			# if self.train:
			# 	return {'img': img, 'img_mask': img_mask, 'cls_name': cls_name, 'anomaly': np.array(anomaly), 'img_path': img_path, 'sample_anomaly': sample_anomaly, 'label': label, 'aug_img': aug_img}
			# else:
			# 	return {'img': img, 'img_mask': img_mask, 'cls_name': cls_name, 'anomaly': np.array(anomaly), 'img_path': img_path, 'sample_anomaly': sample_anomaly, 'label': label}
		...
		if self.use_sample == True:
			if self.train:
				...
				for i in range(5):
					...
					image, augmented_image, anomaly_mask, has_anomaly = self.transform_image(img_path[i],
																							self.anomaly_source_paths[
																								anomaly_source_idx])
					
     
					has_anomaly = int(has_anomaly[0])
     
					# if has_anomaly:
					# 	from torchvision.utils import save_image
					# 	save_image(torch.tensor(image).unsqueeze(0), 'example_image.png')
					# 	save_image(torch.tensor(augmented_image).unsqueeze(0), 'example_image_noise.png')
					# 	import ipdb; ipdb.set_trace()
					
					img.append(torch.tensor(image).unsqueeze(0))
					augmented_img.append(torch.tensor(augmented_image).unsqueeze(0))
					img_mask.append(torch.tensor(anomaly_mask).unsqueeze(0))
					anomaly.append(has_anomaly)
				#import ipdb; ipdb.set_trace()
				sample_anomaly = 0 if 1 not in anomaly else 1
				img = torch.cat(img, dim=0)
				augmented_img = torch.cat(augmented_img, dim=0)
				img_mask = torch.cat(img_mask, dim=0)
				return {'img': img, 'img_mask': img_mask, 'augmented_image': augmented_img, 'cls_name': cls_name, 'anomaly': np.array(anomaly), 'img_path': img_path, 'sample_anomaly': sample_anomaly}
				...
				img_paths, mask_path, cls_names, specie_name, anomalys = [], [], [], [], []
				img, img_mask=[], []
				...
				for data in sample:
					...
					img.append(torch.tensor(image).unsqueeze(0))
					img_mask.append(torch.tensor(mask).unsqueeze(0))
					cls_names.append(cls_name)
					anomalys.append(anomaly)
					img_paths.append(img_path)
				#import ipdb; ipdb.set_trace()
				sample_anomaly = 0 if 1 not in anomalys else 1
				img = torch.cat(img, dim=0)
				img_mask = torch.cat(img_mask, dim=0)
				return {'img': img, 'img_mask': img_mask, 'cls_name': cls_names, 'anomaly': np.array(anomalys), 'img_path': img_paths, 'sample_anomaly': sample_anomaly}
...
class Realnet(data.Dataset):
	...
	def choice_anomaly_type(self):
		# if 'normal' in self.anomaly_types.keys():
		# 	#import ipdb; ipdb.set_trace()
		...
		if self.use_sample == True:
			if self.train:
				...
				for i in range(5):
					...
					# from torchvision.utils import save_image
					# save_image(torch.tensor(gt_image).unsqueeze(0), 'example_image.png')
					# save_image(torch.tensor(image).unsqueeze(0), 'example_image_noise.png')
					# import ipdb; ipdb.set_trace()
				...
				img_mask = torch.cat(img_masks, dim=0)
				return {'img': img, 'img_mask': img_mask, 'gt_image': gt_image, 'cls_name': cls_name, 'anomaly': np.array(anomaly), 'img_path': img_path, 'sample_anomaly': sample_anomaly}
				...
				img = torch.cat(imgs, dim=0)
				img_mask = torch.cat(img_masks, dim=0)
				return {'img': img, 'img_mask': img_mask, 'cls_name': cls_name, 'anomaly': np.array(anomaly), 'img_path': img_path, 'sample_anomaly': sample_anomaly}
