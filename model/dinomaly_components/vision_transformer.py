...
class LinearAttention2(nn.Module):
    ...
    def forward(self, x, attn_mask=None, rope=None):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4) # (B, N, 3, self.num_heads, C // self.num_heads)
        q, k, v = qkv[0], qkv[1], qkv[2] # (B, num_heads, N, head_dim)

        q = nn.functional.elu(q) + 1. # ELU激活函数+1保证非负
        k = nn.functional.elu(k) + 1.# ELU激活函数+1保证非负

        kv = torch.einsum('...sd,...se->...de', k, v) # (B, num_heads, head_dim, head_dim)
        z = 1.0 / torch.einsum('...sd,...d->...s', q, k.sum(dim=-2)) # (B, num_heads, N)
        x = torch.einsum('...de,...sd,...s->...se', kv, q, z) # B, num_heads, N, head_dim
