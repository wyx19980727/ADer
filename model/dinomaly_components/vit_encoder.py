...
def load(name):
    ...
    if "dino" in name:
        if "v2" in name:
            ...
            # model = vision_transformer_dinov3.__dict__[f'vit_{arch}']()

            # if arch == "base":
            #     ckpt_pth = download_cached_file(
            #         f"https://dl.fbaipublicfiles.com/dinov3/dinov3_vitb{patchsize}/dinov3_vitb{patchsize}_pretrain.pth")
            # elif arch == "small":
            #     ckpt_pth = "/home/albus/Python_Codes/ADer/ader_weights/dinov3_vits16_pretrain_lvd1689m-08c60483.pth"
            # else:
            #     raise ValueError("Invalid type of architecture. It must be either 'small' or 'base'.")
            ...
            model = torch.hub.load(REPO_DIR, f'dinov3_vits16', source='local', weights=weight_path)
