...
class AFS(nn.Module):
    ...
    def set_input(self, inputs):
        ...
        inputs['img_mask'] = inputs['img_mask'].cuda()
        if 'gt_image' in inputs.keys():
            inputs['gt_image'] = inputs['gt_image'].cuda()
        else:
            inputs['gt_image'] = None
        if len(inputs['img'].shape)==5:
            inputs['gt_image'] = inputs['gt_image'].flatten(0, 1)
            inputs['img'] = inputs['img'].flatten(0, 1)
            inputs['img_mask'] = inputs['img_mask'].flatten(0, 1)

        
        inputs['cls_name'] = inputs['cls_name']
        inputs['cls_name'] = np.array(inputs['cls_name'])
        inputs['cls_name'] = np.transpose(inputs['cls_name'], (1, 0))
        inputs['cls_name'] = inputs['cls_name'].flatten()

        inputs['anomaly'] = inputs['anomaly'].flatten(0, 1)

        inputs['img_path'] = inputs['img_path']
        inputs['img_path'] = np.array(inputs['img_path'])
        inputs['img_path'] = np.transpose(inputs['img_path'], (1, 0))
        inputs['img_path'] = inputs['img_path'].flatten()

        # self.sample_anomaly = inputs['sample_anomaly']
        inputs['bs'] = inputs['img'].shape[0]
