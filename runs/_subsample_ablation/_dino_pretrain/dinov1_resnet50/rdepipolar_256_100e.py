...
class cfg(cfg_common, cfg_dataset_default, cfg_model_rd):
	...
	def __init__(self):
		...
		self.model_t.name = 'timm_resnet50'
		self.model_t.kwargs = dict(pretrained=True, checkpoint_path='ader_weights/dino_resnet50_pretrain.pth',
								   strict=False, features_only=True, out_indices=[1, 2, 3])
		self.model_s = Namespace()
		self.model_s.name = 'de_resnet50'
		self.model_s.kwargs = dict(pretrained=False, checkpoint_path='', strict=False)
		...
		# self.model.kwargs = dict(pretrained=True, checkpoint_path='runs/_full_ablation/_full_img_size/RDUniADEpipolar_16*16_matchanything_FM_RANSAC_512_448_train/net.pth', strict=True, model_t=self.model_t,
		# 						 model_s=self.model_s)
		...
		# ==> trainer
		self.trainer.name = 'RDEpipolarTrainer'
