...
class cfg(cfg_common, cfg_dataset_default, cfg_model_uniad):
	...
	def __init__(self):
		...
		self.trainer.checkpoint = 'runs_subsample'
		...
		# self.model.kwargs = dict(pretrained=True, checkpoint_path='/home/albus/Python_Codes/ADer/ader_weights/uniad_realiad_net_100.pth', strict=True, model_backbone=self.model_backbone,
		...
		self.loss.loss_terms = [
			dict(type='L2Loss', name='pixel'),
			dict(type='SupervisedContrastiveLoss', name='contrastive'),
		]
		...
		self.logging.log_terms_train = [
			...
			dict(name='contrastive', suffixes=[''], fmt=':>5.3f', add_name='avg'),
		]
