...
class DeSTSegTrainer(BaseTrainer):
    def __init__(self, cfg):
        ...
        #     nesterov=False,
        # )
        self.optim.seg_optimizer = torch.optim.AdamW(
            [{"params": self.net.destseg.segmentation_net.res.parameters(), "lr": cfg.lr*10},
                {"params": self.net.destseg.segmentation_net.head.parameters(), "lr": cfg.lr},],
            betas=(0.9, 0.999), eps=1e-8, weight_decay=0.0001, amsgrad=False
        )
        print('optim finish!')
    ...
    def set_input(self, inputs):
        ...
        if self.imgs is None:
            ...
            self.imgs = self.imgs.cuda()
        self.imgs_mask = inputs['img_mask'].cuda()
        if len(self.ori_imgs.shape)==5:
            self.ori_imgs = self.ori_imgs.flatten(0, 1)
        ...
        self.cls_name = self.cls_name.flatten()

        self.anomaly = inputs['anomaly'].flatten(0, 1)
