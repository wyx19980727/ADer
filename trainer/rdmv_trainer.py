...
class RDMVTrainer(BaseTrainer):
	def __init__(self, cfg):
		super(RDMVTrainer, self).__init__(cfg)
	...
	def forward(self):
		...
		self.feats_t, self.feats_s, self.feats_t_grid, self.glb_feats= self.net(self.imgs)
	...
	def optimize_parameters(self):
		...
		with self.amp_autocast():
			...
			loss_glb = self.loss_terms['glb'](self.glb_feats, self.labels)
			loss_lcl = self.loss_terms['lcl'](self.feats_t, self.feats_t_grid, self.feats_s, self.labels, self.img_path)
   
		loss = loss_cos + loss_glb + loss_lcl
		...
		update_log_term(self.log_terms.get('glb'), reduce_tensor(loss_glb, self.world_size).clone().detach().item(), 1, self.master)
		update_log_term(self.log_terms.get('lcl'), reduce_tensor(loss_lcl, self.world_size).clone().detach().item(), 1, self.master)
