...
class SimpleNetTrainer(BaseTrainer):
	...
	def test(self):
		...
		results=dict(
			...
			anomaly_scores=np.empty((test_img_length, ), dtype='float32'),
		...
		while batch_idx < test_length:
			...
			if len(test_data['img'].shape) == 5:
				...
				if len(test_data['img']) == self.cfg.batch_test_per:
					...
					results['anomaly_scores'][5*batch_idx*self.cfg.batch_test_per:5*(batch_idx+1)*self.cfg.batch_test_per] = anomaly_score
    			# 2. last batch
				else:
					#import ipdb; ipdb.set_trace()
					...
					results['anomaly_scores'][5*batch_idx*self.cfg.batch_test_per:] = anomaly_score
				...
				if len(test_data['img']) == self.cfg.batch_test_per:
					...
					results['sample_anomaly'][batch_idx*self.cfg.batch_test_per:(batch_idx+1)*self.cfg.batch_test_per] = self.sample_anomaly
					results['anomaly_scores'][batch_idx*self.cfg.batch_test_per:(batch_idx+1)*self.cfg.batch_test_per] = anomaly_score
    			# 2. last batch
				else:
					# import ipdb; ipdb.set_trace()
					...
					results['sample_anomaly'][batch_idx*self.cfg.batch_test_per:] = self.sample_anomaly
					results['anomaly_scores'][batch_idx*self.cfg.batch_test_per:] = anomaly_score
