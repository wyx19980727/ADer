...
class UniADTrainer(BaseTrainer):
	...
	def optimize_parameters(self):
		...
		with self.amp_autocast():
			...
			# tmp = 0
			# label_list = []
			# for i in range(self.feats_x.shape[0]):
			# 	if i % 5 == 0:
			# 		tmp += 1
			# 	label_list.append(tmp)
			# label = torch.tensor(label_list, device=self.feats_x.device)
			# loss_contrast = self.loss_terms['contrastive'](self.feats_x, label)
			# loss = loss_mse + loss_contrast
		self.backward_term(loss_mse, self.optim)
		# self.backward_term(loss_contrast, self.optim)
