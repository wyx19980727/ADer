...
class Evaluator(object):
    ...
    def run(self, results, cls_name, logger=None):
        ...
        if self.use_adeval:
            ...
            # Every 5 element max
            assert len(pr_sp_max) % 5 == 0
            assert len(gt_sp) % 5 == 0
            pr_sample_max = pr_sp_max.reshape(-1, 5).max(axis=1)
            gt_sample = gt_sp.reshape(-1, 5).max(axis=1)
            accum.add_sample(torch.tensor(pr_sample_max), torch.tensor(gt_sample))
        ...
        for metric in self.metrics:
            ...
            elif metric.startswith('S_AUROC'):
                metric_results[metric] = metrics['s_auroc']
            elif metric.startswith('S_AUPR'):
                metric_results[metric] = metrics['s_aupr']
